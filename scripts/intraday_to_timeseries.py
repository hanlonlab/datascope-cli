...
nyse_schedule = nyse.schedule(start_date="2005-01-01", end_date="2024-12-31")
nasdaq = mcal.get_calendar("NASDAQ")
nasdaq_schedule = nasdaq.schedule(start_date="2005-01-01", end_date="2024-12-31")
cboe_equity = mcal.get_calendar("CBOE_Equity_Options")
cboe_equity_schedule = cboe_equity.schedule(start_date="2005-01-01", end_date="2024-12-31")

# Get most expansive of the three calendars
schedule = pd.concat([nyse_schedule, nasdaq_schedule, cboe_equity_schedule],axis=1)
schedule = pd.concat([schedule["market_open"].groupby(level=0, axis=1).min(),schedule["market_close"].groupby(level=0, axis=1).max()],axis=1)
...
for chunk in df_chunks:
    ...
    chunk = pd.merge(chunk, schedule, left_on="Date", right_index=True, how="left")
